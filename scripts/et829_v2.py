...
def trim_uninit(raw, run_len=15):
    ...
    Signals passing through 0x00 (e.g. ramps) are safe: they only produce
    short isolated zeros, never a run of 15+."""
    n = len(raw)
...
def trim_head_zeros(raw, min_run=5):
    ...
    Catches short uninit heads that trim_uninit (run_len=15) would miss."""
...
def trim_interior_zeros(raw, skip_pct=0.15, run_len=10):
    """Find the first run of >=run_len zeros AFTER the first skip_pct of the
    buffer and cut there. Removes the uninit zero block that sits between the
    real signal and the noise tail (e.g. square wave ends at sample 600, then
    600 zeros, then sparse noise at end of buffer)."""
    n = len(raw)
...
def trim_noise_tail(raw, diff_thresh=40, tail_pct=0.20, min_cluster=3):
    """Remove random ADC noise burst at the tail of a capture.
    Only examines the LAST tail_pct of the buffer — this prevents false-cutting
    legitimate signal edges (e.g. square wave transitions, diff≈57) in the body.
    Works on the original raw frame before interior zeros are stripped."""
    n = len(raw)
    if n < 20: return raw
...
def trim_channel_bleed(raw, win=8, variance_ratio=5.0, max_body_var=80, flat_tol=4):
    ...
    The device stores CH1_block then CH2_block sequentially; when the block boundary
    doesn't align with our midpoint split, the other channel's data bleeds in at the tail.
    Only acts when the body (first 60%) is nearly flat (low variance) AND the tail
    region has significantly higher variance — i.e. a flat channel contaminated by
    an active sine or ramp from the other channel.
    Uses a run of 'win' consecutive flat samples as the true end-of-signal marker."""
    n = len(raw)
    ...
    if body_var > max_body_var: return raw  # active signal — don't cut

    tail_start = int(n * 0.8)
    tail_var = sum((v - body_mean)**2 for v in raw[tail_start:]) / (n - tail_start)
    if tail_var <= body_var * variance_ratio: return raw  # tail also flat, no bleed

    # Find the last run of 'win' consecutive samples within flat_tol of body_mean
...
def clean_single(raw):
    ...
    Order is critical: noise trim must run BEFORE interior-zero trim so it
    operates on the original frame and won't confuse signal edges with noise."""
    r = trim_head_zeros(raw)      # (1) skip leading uninit zeros (run >= 5)
    r = trim_uninit(r)            # (2) standard edge cleanup (run = 15)
    r = trim_noise_tail(r)        # (3) tail noise burst — before interior zeros
    r = trim_interior_zeros(r)    # (4) cut uninit zero block (e.g. after square wave)
    r = trim_uninit(r)            # (5) final edge cleanup
    return r

def clean_dual(raw_ch1, raw_ch2):
    """Cleaning pipeline for the two halves of a dual-channel frame.
    Adds bleed detection: removes other-channel data that leaked into the tail."""
