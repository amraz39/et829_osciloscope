...
def trim_noise_tail(raw, diff_thresh=40, cluster_gap=100, min_cluster=3):
    """
    Remove noise burst at the tail of a waveform capture.
    After the real signal ends, the device buffer contains random ADC garbage —
    large sample-to-sample spikes. Scan backwards to find the last such cluster
    and cut before it.
    diff_thresh:  min diff between consecutive samples to be 'noisy'
    cluster_gap:  max quiet samples bridged within a single noise cluster
    min_cluster:  min number of noisy samples to qualify as a real burst
    """
    n = len(raw)
    if n < 10: return raw
    diffs = [abs(raw[i] - raw[i-1]) for i in range(1, n)]
    noisy = [1 if d > diff_thresh else 0 for d in diffs]
    last_noisy = next((i for i in range(n-2, -1, -1) if noisy[i]), -1)
    if last_noisy < 0: return raw
    cluster_start = last_noisy
    gap = 0
    for i in range(last_noisy-1, -1, -1):
        if noisy[i]:
            cluster_start = i
            gap = 0
        else:
            gap += 1
            if gap > cluster_gap: break
    if sum(noisy[cluster_start:last_noisy+1]) < min_cluster: return raw
    return raw[:cluster_start]
...
def parse_scope_frame(data):
    ...
    try:
        ...
        if ch_flags == 0x03 and len(raw_all) >= 2:
            mid     = len(raw_all) // 2
            raw_ch1 = trim_noise_tail(trim_uninit(raw_all[:mid]))
            raw_ch2 = trim_noise_tail(trim_uninit(raw_all[mid:]))
        elif ch_flags == 0x02:
            raw_ch1 = []
            raw_ch2 = trim_noise_tail(trim_uninit(raw_all))
        else:
            raw_ch1 = trim_noise_tail(trim_uninit(raw_all))
            raw_ch2 = []
