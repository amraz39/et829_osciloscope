...
def trim_uninit(raw, run_len=15):
    """Remove uninitialized SRAM zeros (0x00) from BOTH edges of a buffer."""
    n = len(raw)
    if n < run_len: return raw
    start = 0
    while start <= n - run_len:
        if all(raw[j] == 0 for j in range(start, start + run_len)): start += 1
        else: break
    end = n
    while end - run_len >= start:
        if all(raw[j] == 0 for j in range(end - run_len, end)): end -= 1
        else: break
    return raw[start:end]

def trim_head_zeros(raw, min_run=5):
    """Skip ALL leading 0x00 bytes if there are at least min_run of them."""
    nz = next((i for i in range(len(raw)) if raw[i] != 0), len(raw))
    return raw[nz:] if nz >= min_run else raw

def trim_interior_zeros(raw, skip_pct=0.15, run_len=10):
    """Cut at first interior zero run after skip_pct of the buffer.
    Removes uninit zeros between signal end and noise tail."""
    n = len(raw)
    skip = max(run_len, int(n * skip_pct))
    for i in range(skip, n - run_len):
        if all(raw[j] == 0 for j in range(i, i + run_len)):
            return raw[:i]
    return raw

def trim_noise_tail(raw, diff_thresh=40, tail_pct=0.20, min_cluster=3,
                    min_first_pct=0.0):
    """Remove random ADC noise burst at the tail.
    Only examines the last tail_pct of the buffer to avoid false-cutting
    periodic signal transitions (square wave edges, diff≈57).
    min_first_pct: second-pass guard — only cut if the first noisy diff is
    after this fraction of the tail, meaning noise is end-concentrated not spread."""
    n = len(raw)
    if n < 20: return raw
    tail_start = max(1, int(n * (1 - tail_pct)))
    tail = raw[tail_start:]
    diffs = [abs(tail[i] - tail[i-1]) for i in range(1, len(tail))]
    noisy = [1 if d > diff_thresh else 0 for d in diffs]
    last_noisy = next((i for i in range(len(noisy)-1, -1, -1) if noisy[i]), -1)
    if last_noisy < 0 or sum(noisy[:last_noisy+1]) < min_cluster: return raw
    first_noisy = next(i for i in range(len(noisy)) if noisy[i])
    if min_first_pct > 0 and first_noisy / max(1, len(diffs)) < min_first_pct:
        return raw  # transitions spread throughout → real signal, not tail noise
    return raw[:tail_start + first_noisy]

def trim_near_zero_tail(raw, threshold=20, min_run=5):
    """Trim trailing near-zero samples (raw < threshold).
    Catches uninit ADC bytes that are close-to-but-not-exactly zero —
    these are missed by trim_uninit which requires exact 0x00 runs.
    Only trims if at least min_run consecutive near-zero samples are found."""
    n = len(raw)
    count_low = 0
    for i in range(n - 1, -1, -1):
        if raw[i] < threshold:
            ...
            if count_low >= min_run:
                return raw[:i + 1]
            break
    return raw

def trim_channel_bleed(raw, win=8, variance_ratio=5.0, max_body_var=80, flat_tol=4):
    """Remove bleed from the other channel at the tail of a dual-channel half-buffer.
    Only acts when body is flat (low variance) but tail has high variance."""
    n = len(raw)
    if n < win * 6: return raw
    body_end = int(n * 0.6)
    body = raw[:body_end]
    body_mean = sum(body) / len(body)
    body_var  = sum((v - body_mean)**2 for v in body) / len(body)
    if body_var > max_body_var: return raw
    tail_start = int(n * 0.8)
    tail_var = sum((v - body_mean)**2 for v in raw[tail_start:]) / (n - tail_start)
    if tail_var <= body_var * variance_ratio: return raw
    for i in range(n - 1, int(n * 0.5), -1):
        if i - win + 1 < 0: break
        if all(abs(raw[j] - body_mean) <= flat_tol for j in range(i - win + 1, i + 1)):
            return raw[:i + 1]
    return raw

def clean_single(raw):
    """Full cleaning pipeline for a single-channel buffer.
    Pass order: (1) head zeros  (2) edge uninit  (3) tail noise burst
                (4) interior zeros  (5) edge cleanup
                (6) second noise pass (end-concentrated only)
                (7) near-zero residual cleanup"""
    r = trim_head_zeros(raw)
    r = trim_uninit(r)
    r = trim_noise_tail(r)                     # pass 1: large bursts in original frame
    r = trim_interior_zeros(r)                 # cut uninit zero block
    r = trim_uninit(r)
    r = trim_noise_tail(r, min_first_pct=0.50) # pass 2: end-concentrated bursts only
    r = trim_near_zero_tail(r)                 # pass 3: residual near-zero bytes
    return r

def clean_dual(raw_ch1, raw_ch2):
    """Cleaning pipeline for dual-channel frame halves with bleed detection."""
    r1 = trim_channel_bleed(clean_single(raw_ch1))
    r2 = trim_channel_bleed(clean_single(raw_ch2))
    return r1, r2
...
def parse_scope_frame(data):
    ...
    try:
        ...
        if ch_flags == 0x03 and len(raw_all) >= 2:
            mid = len(raw_all) // 2
            raw_ch1, raw_ch2 = clean_dual(raw_all[:mid], raw_all[mid:])
        elif ch_flags == 0x02:
            raw_ch1 = []
            raw_ch2 = clean_single(raw_all)
        else:
            raw_ch1 = clean_single(raw_all)
            raw_ch2 = []
...
# The device ADC maps 25 counts per division (200 of 256 counts across 8 divisions).
# Formula: voltage = (raw - 128) / CPD * vdiv
COUNTS_PER_DIV = 25

def raw_to_volts(raw_samples, vdiv, ac_couple=False):
    if not raw_samples: return []
    v = [(s - 128) / COUNTS_PER_DIV * vdiv for s in raw_samples]
    if ac_couple:
        dc = sum(v) / len(v)
        v = [x - dc for x in v]
    return v
...
def estimate_freq(volts, tb_ms_per_div, samples_per_div=60):
    """
    Estimate frequency from upward zero crossings.
    tb_ms_per_div: time/div in ms (user supplied). If None, returns (None, num_cycles).
    samples_per_div: empirical — 600 samples / 10 div = 60 sa/div.
    """
    if not volts or len(volts) < 4:
        return None, 0
    mean = sum(volts) / len(volts)
    c = [v - mean for v in volts]
    crossings = sum(1 for i in range(1, len(c)) if c[i-1] < 0 and c[i] >= 0)
    if tb_ms_per_div is None or crossings == 0:
        return None, crossings
    sample_period_s = (tb_ms_per_div * 1e-3) / samples_per_div
    total_time_s = sample_period_s * len(volts)
    return crossings / total_time_s, crossings
...
def style_ax(ax, vdiv, n, title='', center=0.0):
    half = vdiv * 4  # 4 divisions above/below centre (display scale)
...
def plot_single(slot_idx, frame, outpath, tb_ms_per_div=None):
    vdiv     = frame['ch1_vdiv']
    ch1      = frame.get('ch1', [])
    ch2      = frame.get('ch2', [])
    n        = frame['n']
    ...
    if has_ch1:
        ...
        freq1, nc1 = estimate_freq(ch1, tb_ms_per_div)
        f1str = f"  F≈{freq1:.1f}Hz" if freq1 else (f"  ~{nc1}cyc" if nc1 > 0 else "")
        stats_lines += [f"CH1  Vpp={v1pp:.3f}V  DC={v1dc:.3f}V  Vrms={v1rms:.3f}V{f1str}"]
    if has_ch2:
        v2pp, v2dc, v2rms, v2min, v2max = wstats(ch2)
        freq2, nc2 = estimate_freq(ch2, tb_ms_per_div)
        f2str = f"  F≈{freq2:.1f}Hz" if freq2 else (f"  ~{nc2}cyc" if nc2 > 0 else "")
        stats_lines += [f"CH2  Vpp={v2pp:.3f}V  DC={v2dc:.3f}V  Vrms={v2rms:.3f}V{f2str}"]
    ax.text(0.99, 0.97, '\n'.join(stats_lines),
...
def cmd_scope(dev, args):
    ...
    def freeze_and_scan(ch_cmd):
        ...
        caller's job). Stops after 4 consecutive USB non-responses.

        NOTE: The ring buffer is frozen when the CH1 button is pressed. After that,
        ALL channel-select commands (0D 02, 0D 03, etc.) return the SAME frozen data —
        this is expected device behaviour, not a bug.

        NOTE: Print order — the code scans seeks 00, 01, 02, ... in order. The device
        ring buffer pointer starts at the NEXT WRITE SLOT (e.g. slot 04 after 4 saves).
        Empty slots (>= pointer) respond first, then the real data slots wrap around.
        We collect all results and print in seek order so the display matches the scan.
        """
        try:
            ...
            return []
        pages = []
        seek_log = []   # collect (seek, frame_or_None, raw_data) for ordered printing
        consecutive_misses = 0
        for p in range(MAX_SCAN):
            data  = seek_and_read(dev, p)
            frame = parse_scope_frame(data)
            seek_log.append((p, frame, data))
            if frame:
                ...
                if consecutive_misses >= 4:
                    break
        # Print all seeks in order (data and misses together)
        for p, frame, data in seek_log:
            if frame:
                pass  # printed by caller with NEW/dup status
            elif data:
    ...
    for idx, (seek, frame) in enumerate(pages):
        ...
        if not args.no_single:
            ...
            plot_single(idx, frame, png,
                         tb_ms_per_div=getattr(args, "tb_ms_per_div", None))
...
def interactive_menu(dev):
    while True:
        print()
        cp(BLD+WHT, "╔════════════════════════════════════════════════╗")
        cp(BLD+WHT, "║   ET829 / MDS8209  -  USB Tool  [v5.0]         ║")
        cp(BLD+WHT, "╠════════════════════════════════════════════════╣")
        ...
        if choice == '0':
            ...
            try:
                ...
                tb_val    = None  # Timebase not stored in saves; show cycle count only
                ac_in     = input("  AC couple (remove DC offset)? [Y/n]: ").strip().lower() != 'n'
                csv_in    = input("  Also export CSV? [Y/n]: ").strip().lower() != 'n'
            ...
            a = ScopeArgs()
            a.out = out_dir; a.vdiv = vdiv_val; a.tb_ms_per_div = tb_val; a.ac = ac_in
